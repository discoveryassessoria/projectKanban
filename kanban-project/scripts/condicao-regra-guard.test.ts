...
sec('3 — estrutura & legados removidos')
{
  ...
  // Editor enxuto em 4 seções (spec) — não mais 9 etapas fragmentadas.
  ok('editor em 4 seções (Stepper)', tab.includes('Stepper') && tab.includes("'Identificação', 'Parcelamento e vencimentos', 'Aplicabilidade e encargos', 'Revisão'"))
  ok('mantém os blocos essenciais reagrupados', tab.includes("titulo=\"Revisão\"") && tab.includes('Política de Taxas') && tab.includes('titulo="Cronograma"') && tab.includes('titulo="Parcelamento e entrada"'))
  ok('consome enums da fonte única', tab.includes('condicao-constants'))
  ok('carteira SUGERIDA (não obrigatória)', tab.includes('Carteira sugerida'))
  ok('Política de Taxas (não "Aplicar taxas")', tab.includes('Política de Taxas') && !tab.includes('Aplicar taxas nesta condição'))
  // "Forma sugerida" (campo único, na Identificação) virou o par
  // Formas permitidas (N:N) + Forma padrão (única, opcional) na etapa Formas.
  ok('campo único "Forma sugerida" removido', !tab.includes('Forma sugerida'))
  ok('Forma padrão é SUGESTÃO, não restrição', tab.includes('Forma padrão') && tab.includes('poderá ser alterada por outra forma permitida'))
