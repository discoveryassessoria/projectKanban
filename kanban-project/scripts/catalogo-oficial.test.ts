...
const modal = ler('src/components/financeiro/v3/LancamentoManualModal.tsx')
ok(!/PRODUTO:|HONORARIO:/.test(modal), 'o modal de lançamento não tem rótulo de cadastro eliminado')
ok(!/filter\([^)]*natureza[^)]*(HONORARIO|PRODUTO)/.test(modal), 'o frontend não faz correção cosmética da lista')
