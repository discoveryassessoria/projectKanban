...
async function main() {
  ...
  const p1 = await prisma.processo.create({
    data: { nome: "TESTE-VERSAO P1", arvoreId: arv.id, workflowRuntime: "v2", faseAtualKey: "analise_documental", tipoProcessoMotorId: tipo?.id ?? null, modalidadeId: habilitacao?.modalidadeId ?? null },
    select: { id: true },
  })
  ...
  const p2 = await prisma.processo.create({
    data: { nome: "TESTE-VERSAO P2", arvoreId: arv.id, workflowRuntime: "v2", faseAtualKey: "analise_documental", tipoProcessoMotorId: tipo?.id ?? null, modalidadeId: habilitacao?.modalidadeId ?? null },
    select: { id: true },
  })
