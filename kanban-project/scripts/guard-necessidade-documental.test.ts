...
const AUTORIZADOS_FORA_DO_RUNTIME: Record<string, string> = {
  ...
  "scripts/etapa6-circuito-completo.test.ts":
    "Etapa 6 (circuito operacional completo): cria e derruba o PRÓPRIO cenário " +
    "(marca ETAPA6-TEST) só no banco de teste",
  ...
  "scripts/palco-retrocesso-motor.ts":
    ...
    "visita anterior e outra nascida depois, para que a reentrada tenha o que preservar e o " +
    "que exigir",
  ...
  "scripts/passo-tarefa-coerencia.test.ts":
    "coerência passo × tarefa: cria a PRÓPRIA obrigação (marca SYNC) só no banco de teste — " +
    "o passo `localizar_registro` opera por NECESSIDADE, e sem obrigação real o motor " +
    "recusa materializar (corretamente), deixando o palco sem par para verificar",
  ...
  "scripts/prova-pasta-documental-linha-do-tempo.test.ts":
    "mandato 'pasta única, uma linha do tempo, não documento por documento' (24/09/2026): monta e derruba o PRÓPRIO cenário (marca PASTALINHATEMPO), só no banco de teste — a pasta agregada (montarPastaDocumental) precisa de uma NecessidadeDocumental real por documento para o passo existir; a necessidade é insumo do palco, nunca o que está sendo provado",
  ...
  "scripts/minha-operacao-semantica.test.ts":
    "correção executiva (Minha Operação, 15/09/2026): prova próxima ação humana, " +
    "humanização de risco, filtros server-side e EMI-021/022 — monta e derruba o PRÓPRIO " +
    "cenário só no banco de teste; a necessidade é criada direto para montar o palco " +
    "mínimo, a materialização em si usa garantirTarefaDePasso (o caminho oficial)",
