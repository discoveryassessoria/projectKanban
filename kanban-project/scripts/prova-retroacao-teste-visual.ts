// scripts/prova-retroacao-teste-visual.ts
// ============================================================================
...
//
// OPERADOR DE PRODUÇÃO (não é *.test.ts de propósito — a trava do projeto,
// exigirBancoDeTeste, recusa QUALQUER script que a chame contra produção por
// desenho, incidente real de 08/08/2026 documentado em _banco-de-teste.ts;
// a porta sancionada para um ato deliberado em produção é
// exigirConfirmacaoDeEscritaEmProducao, usada aqui).
//
//   EU_CONFIRMO_ESCRITA_EM_PRODUCAO=1 npx tsx scripts/prova-retroacao-teste-visual.ts
// ============================================================================
...
// NOMES EXATOS, NUNCA PREFIXO — "[TESTE VISUAL]" sozinho colidiria com outros
// processos sintéticos que a própria auditoria já criou em produção (ex.:
// processo 632, "[TESTE VISUAL] [TESTE A] — Prazo vencido"), que este script
// NÃO pode tocar. Só os DOIS nomes que ESTE script cria são elegíveis a limpeza.
const NOMES_SINTETICOS_DESTE_SCRIPT = [`[TESTE VISUAL] Em Andamento`, `[TESTE VISUAL] Finalizado`]

async function limparFixtureSintetica() {
  const procs = await prisma.processo.findMany({ where: { nome: { in: NOMES_SINTETICOS_DESTE_SCRIPT } }, select: { id: true } })
  ...
  // NÃO apaga o usuário admin@cfgretro.test: as revisões/PHASE_* que ele publicou em
  // TESTEVIS_fase ficam no histórico de propósito (auditoria real), e apagar o autor
  // depois deixa a referência órfã ("usuário removido (#N)") — achado real desta
  // rodada. Preserva o responsável de qualquer ação já auditada.
}

async function main() {
  exigirConfirmacaoDeEscritaEmProducao("Prova ponta a ponta da retroação (item 5, mandato Catálogo de Fases) — fixture isolada TESTEVIS_fase, self-restoring, zero cliente real", "prova-retroacao-teste-visual")
  ...
  const admin = await prisma.usuario.upsert({
    where: { email: "admin@cfgretro.test" },
    update: {},
    create: { nome: "Admin CFGRETRO", email: "admin@cfgretro.test", senha: "x", tipo: "admin" },
    select: { id: true },
  })
  ...
  // Fixture da fase ANTES por prisma direto — não pelo materializador canônico:
  // a latência de rede até o Postgres remoto (pooled.db.prisma.io) estourava
  // repetidamente o timeout de 5s da transação interativa de
  // `garantirTarefaDePasso` (achado real, 3 tentativas). O que este teste prova
  // é a RECONCILIAÇÃO de TESTEVIS_fase (seção 4+), não a materialização da fase
  // ANTES — que só precisa existir como fato prévio a preservar.
  const chaveWfAntes = montarChaveWorkflow({ processoId: procEmAndamento.id, faseMacroKey: PHASE_KEY_ANTES, workflowDefinitionId: wfAntes.id, workflowVersion: 1, ciclo: 1 })
  const instAntes = await prisma.phaseWorkflowInstance.create({
    data: { processoId: procEmAndamento.id, faseMacroKey: PHASE_KEY_ANTES, workflowDefinitionId: wfAntes.id, workflowVersion: 1, ciclo: 1, status: "ATIVO", origem: "MOTOR", chaveIdempotencia: chaveWfAntes },
  })
  const chavePassoAntes = montarChavePasso({ workflowInstanceId: instAntes.id, stepDefinitionId: 999999, stepKey: "passo", stepDefinitionVersion: 1, ciclo: 1 })
  const stepAntes = await prisma.phaseWorkflowStepInstance.create({
    data: { workflowInstanceId: instAntes.id, processoId: procEmAndamento.id, faseMacroKey: PHASE_KEY_ANTES, stepDefinitionId: 999999, stepDefinitionVersion: 1, stepKey: "passo", ordem: 1, tipo: "HUMANO", obrigatorio: true, geraTarefa: true, ciclo: 1, status: "DISPONIVEL", chaveIdempotencia: chavePassoAntes },
  })
  const tarefaAntes = await prisma.tarefa.create({
    data: { processoId: procEmAndamento.id, faseMacroKey: PHASE_KEY_ANTES, workflowInstanceId: instAntes.id, workflowStepInstanceId: stepAntes.id, titulo: "Passo", statusTarefa: "NAO_INICIADA", origem: "workflow", chaveIdempotencia: `tarefa|${chavePassoAntes}` },
  })
  ok("2.1) processo em andamento tem a fase ANTES materializada (fato prévio a preservar)", !!tarefaAntes.id)
  const tarefaAntesId = tarefaAntes.id
