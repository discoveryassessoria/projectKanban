...
function run() {
  ...
  // MUDOU DE INTENTO em 10/09/2026, aprovado pelo usuário: "Central de
  // Notificações" (abas soltas Prazos/Ações, sem categoria real do motor)
  // deixou de ser o conceito principal. Virou "Central Operacional" —
  // "Minha Central Operacional" pro operacional —, o MESMO motor de
  // /operacao/central (agregacaoPorFamilia/indicadoresGerenciais), sempre
  // agrupado por família, com link para a tela cheia (fase/etapa/condição).
  ok(/isAdmin \? "Central Operacional" : "Minha Central Operacional"/.test(content), "bloco Central Operacional (mesmo motor de /operacao/central)")
  ok(/data\.centralOperacional/.test(content), "consome o mesmo motor agregado por família (nenhuma segunda fonte)")
  ok(/nomeFamilia/.test(content), "resultado agrupado por família, sempre")
  ...
  console.log("\nRegras do conceito:")
  // Vazio da Central Operacional usa um estado compacto com ícone + duas
  // linhas — nunca um EmptyState de corpo inteiro — e a mensagem muda por
  // papel (10/09/2026): "na operação" pro admin, "atribuída a você" pro
  // operacional — nunca a mesma frase pros dois.
  ok(/Nenhuma tarefa aberta na opera[çc][ãa]o/.test(content), "vazio da Central Operacional (admin) é compacto, não bloco vazio")
  ok(/Nenhuma tarefa aberta atribu[íi]da a voc[êe]/.test(content), "vazio da Central Operacional (operacional) nunca mostra frase de admin")
  ok(
  ...
  // MUDOU DE INTENTO em 10/09/2026: a Central Operacional agrupa por família
  // (total/atrasadas/executáveis por família), não mais por nível de
  // prioridade de item solto — `nivelStyle` saiu do bloco principal da Home
  // junto com o antigo "Central de Notificações".
