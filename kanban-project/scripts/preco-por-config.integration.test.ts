...
async function main() {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      // M-UNIFICA — UMA config por mestre; papel (CUSTO/RECEITA) vive em TabelaValor.natureza.
      const cfg = await tx.produtoFinanceiro.findFirst({ where: { possuiCusto: true }, select: { id: true } })
      if (!cfg) throw new Error('sem ProdutoFinanceiro (com custo) para teste')
      ...
      // natureza sempre setada (M-UNIFICA: papel vive em TabelaValor.natureza).
      await tx.tabelaValor.create({ data: { name: 'g', configuracaoFinanceiraItemId: cfg.id, natureza: NaturezaPreco.CUSTO, moeda: Moeda.BRL, valor: 100 } })
      await tx.tabelaValor.create({ data: { name: 'fa', configuracaoFinanceiraItemId: cfg.id, natureza: NaturezaPreco.CUSTO, moeda: Moeda.BRL, valor: 150, fornecedorId: fA.id } })
      await tx.tabelaValor.create({ data: { name: 'p99', configuracaoFinanceiraItemId: cfg.id, natureza: NaturezaPreco.CUSTO, moeda: Moeda.EUR, valor: 90, processoTipoId: '99' } })
      await tx.tabelaValor.create({ data: { name: 'zero', configuracaoFinanceiraItemId: cfg.id, natureza: NaturezaPreco.CUSTO, moeda: Moeda.BRL, valor: 0, fornecedorId: fB.id } })
      ...
      const cfg2 = await tx.produtoFinanceiro.findFirst({ where: { id: { not: cfg.id }, possuiCusto: true }, select: { id: true } })
