...
// A natureza PRODUTO foi ELIMINADA da arquitetura (lib/financeiro/catalogo-oficial):
// o espelho de uma Configuração Financeira sem mestre próprio nasce como OUTRO.
ok(/NaturezaItem\.SERVICO/.test(sync) && /itemCatalogo\.upsert/.test(sync), "7. serviço de sync faz upsert do espelho (natureza SERVICO)")
ok(!/NaturezaItem\.PRODUTO/.test(sync) && /NaturezaItem\.OUTRO/.test(sync), "7b. espelho de config sem mestre nasce OUTRO (PRODUTO é estrutura eliminada)")
...
// R20 — na EDIÇÃO da Configuração Financeira o mestre é IMUTÁVEL: nome/código
// pertencem ao cadastro mestre e a config nunca os reescreve (nem troca de mestre).
ok(!/sincronizarItemDeProduto/.test(pId) && /const nome = atual\.nome/.test(pId), "11. PUT da config NÃO reescreve o mestre (R20: mestre imutável na edição)")
...
// ── Face única do cadastro mestre (UI) ──────────────────────────────────────
// A migração inverteu o sentido do LOTE B: a tela técnica "Catálogo Mestre" saiu
// da navegação e o Catálogo de Serviços passou a ser a ÚNICA face do mestre.
// Não há mais aviso de depreciação apontando o operador para outra tela.
console.log("\nFace única do cadastro mestre:")
const tab = src("src/components/gerenciamentoComponents/ProdutosServicosTab.tsx")
ok(!/Tela legada/.test(tab), "15. tela de Serviços não exibe aviso de tela legada em migração")
ok(/Catálogo de Serviços/.test(tab) && /catalogo-mestre/.test(tab), "16. Catálogo de Serviços é a face única do mestre (lê serviços + itens do mestre)")
