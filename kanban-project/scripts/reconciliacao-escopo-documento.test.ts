...
async function main() {
  ...
  async function criarProcessoComInstanciaLegada(nome: string, statusInstancia: "ATIVO" | "CONCLUIDO", statusTarefa: "NAO_INICIADA" | "CONCLUIDO_RECEBIDO") {
    ...
    const processo = await prisma.processo.create({
      data: { nome: `${MARCA} ${nome}`, workflowRuntime: "v2", faseAtualKey: FASE_ALVO, tipoProcessoMotorId: tipo.id, modalidadeId: oferta.modalidadeId, macroWorkflowVersion: 1, arvoreId: arvore.id },
      select: { id: true },
    })
