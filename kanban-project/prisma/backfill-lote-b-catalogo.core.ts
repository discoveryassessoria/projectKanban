...
// A CATEGORIA não viaja por aqui: ela pertence ao mestre (ItemCatalogo.categoriaId).

import { codeServicoMestre, codeProdutoMestre } from '../src/services/catalogo-helpers'

export interface ServicoRow { id: number; code: string; name: string; itemCatalogoId: number | null }
export interface ProdutoRow { id: number; codigo: string; nome: string; itemCatalogoId: number | null }

export interface UpsertServico { id: number; code: string; name: string }
...
export function planejarServicos(servicos: ServicoRow[]): UpsertServico[] {
  return servicos
    ...
    .map((s) => ({ id: s.id, code: codeServicoMestre(s.code), name: s.name }))
}
