// src/app/api/processos/[processoId]/fase-documental-kpis/[stepKey]/incluir/route.ts
// ============================================================================
// POST — "incluir na pasta" = concluir a subtarefa "preparar_documentos" para
// os documentos escolhidos. Não é uma ação nova: é a MESMA porta de sempre
// (atualizarPassoV2 + subtarefaEsperada), só chamada em lote com o alvo fixo
// em "preparar_documentos" — nunca em qualquer outra subtarefa.
// ============================================================================
...
export async function POST(
  ...
  const resultados: Array<{ documentoId: number; ok: boolean; motivo?: string }> = []
  for (const documentoId of documentoIds) {
    if (!idsValidos.has(documentoId)) { resultados.push({ documentoId, ok: false, motivo: "Documento não pertence a este processo." }); continue }
    try {
      const wf = await garantirOperacaoDocumentoV2(documentoId, ctx)
      const stepInstanceId = wf.workflow?.currentStepId ?? null
      if (!stepInstanceId) { resultados.push({ documentoId, ok: false, motivo: "Sem etapa aberta nesta fase para este documento." }); continue }
      const r = await atualizarPassoV2(documentoId, stepInstanceId, { status: "concluida", subtarefaEsperada: "preparar_documentos" }, ctx)
      resultados.push(r.ok ? { documentoId, ok: true } : { documentoId, ok: false, motivo: r.error })
    } catch (e) {
  ...
  return NextResponse.json({
    ok: true,
    incluidos: resultados.filter((r) => r.ok).length,
