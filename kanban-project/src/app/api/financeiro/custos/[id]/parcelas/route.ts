// src/app/api/financeiro/custos/[id]/parcelas/route.ts
// PATCH /api/financeiro/custos/[id]/parcelas
//
// REPARCELAMENTO OPERACIONAL DE CUSTO (paridade com a receita). Redistribui o MESMO total contratual em N parcelas.
...
// com pagamento registrado. Preserva integralmente as parcelas já liquidadas,
...
export async function PATCH(req: NextRequest, ctx: RouteContext) {
  try {
    ...
    const quitadas = custo.parcelas.filter((p) => p.status === 'RECEBIDA' || p.status === 'PAGA')
    const abertas = custo.parcelas.filter((p) => p.status === 'PENDENTE')

    if (!renegociando && quitadas.length > 0) {
      return NextResponse.json(
        { error: 'O parcelamento não pode ser alterado porque já existe pagamento registrado. Use a renegociação.' },
        { status: 409 },
      )
    }
    if (renegociando) {
    ...
    const totalContratado = Number(custo.valor)
    ...
    const inicio = data1 ?? custo.vencimento
    ...
    if (condicaoId) {
      const { condicao, motivoDescarte } = await condicaoPorId(condicaoId, {
        natureza: 'CUSTO', moeda: String(custo.moeda), total, emDatas: new Date(),
      })
      if (!condicao) {
        return NextResponse.json(
    ...
    const atualizada = await prisma.$transaction(async (tx) => {
      ...
      const offset = renegociando ? Math.max(0, ...custo.parcelas.map((p) => p.numero)) : 0
      ...
      await tx.eventoFinanceiro.create({
        data: {
          ...
          descricao: renegociando
            ? `Renegociação: saldo de ${total.toFixed(2)} ${custo.moeda} em ${plano.length}×${condicaoAplicada ? ` pela condição ${condicaoAplicada}` : ''}. ` +
              `${quitadas.length} parcela(s) liquidada(s) preservada(s), ${abertas.length} encerrada(s). Motivo: ${String(body?.motivo ?? '').trim()}`.slice(0, 400)
            : `Parcelamento alterado para ${plano.length}×${condicaoAplicada ? ` pela condição ${condicaoAplicada}` : ''} (total contratual preservado: ${total.toFixed(2)} ${custo.moeda})`.slice(0, 500),
          valor: total,
        },
      })
      return tx.custo.update({
        where: { id },
        // nParcelas/data1 são campos OPERACIONAIS do parcelamento — valor não muda.
        // O VALOR contratado nunca muda — nem na renegociação.
        data: { nParcelas: (renegociando ? quitadas.length : 0) + plano.length, vencimento: plano[0]?.vencimento ?? inicio },
        include: { parcelas: { orderBy: { numero: 'asc' } } },
      })
    ...
    console.error('[PATCH /api/financeiro/custos/[id]/parcelas] erro:', err)
