...
// ============================================================================
// TELA OFICIAL — RECEITA (Financeiro V3). Reprodução fiel do mockup aprovado.
      ...
      <div className="mx-auto max-w-[1400px] px-8 py-6">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex flex-col items-end gap-4">
            ...
            <div className="flex items-center gap-2">
              <a href={movLink} className="inline-flex items-center gap-2 rounded-lg border border-white/15 bg-[#1b2027] px-3.5 py-2 text-sm text-white/80 hover:border-white/25">Ver movimentações <ExternalLink className="h-3.5 w-3.5" /></a>
              <div className="relative">
                <button onClick={() => setMenuOpen((o) => !o)} className="inline-flex items-center gap-2 rounded-lg border border-white/15 bg-[#1b2027] px-3.5 py-2 text-sm text-white/80 hover:border-white/25">Mais ações <MoreVertical className="h-3.5 w-3.5" /></button>
                {menuOpen && (
                  <div className="absolute right-0 z-20 mt-2 w-52 overflow-hidden rounded-lg border border-white/10 bg-[#1b2027] py-1 shadow-xl">
                    <a href={movLink} onClick={() => setMenuOpen(false)} className="block px-3.5 py-2 text-sm text-white/80 hover:bg-[#252c35]">Ver movimentações</a>
                    <button onClick={() => { copiarCodigo(); setMenuOpen(false) }} className="block w-full px-3.5 py-2 text-left text-sm text-white/80 hover:bg-[#252c35]">Copiar código</button>
                  </div>
                )}
              </div>
        ...
        <div className="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-[1fr_320px]">
          ...
          <div className="space-y-4">
            <Painel titulo="Resumo financeiro" aberto>
              ...
              <Linha k={isCusto ? "Total pago" : "Total recebido"} v={brl(d.resumo.recebidoBrl)} cor="text-[#4ade80]" />
              <Linha k="Saldo a receber" v={brl(d.resumo.saldoBrl)} cor="text-[#7dd3fc]" />
              <Linha k="Descontos" v={brl(d.resumo.descontosBrl)} />
              <Linha k="Ajustes" v={brl(d.resumo.ajustesBrl)} />
