    ...
    <div className="space-y-5">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm text-white/60">
              Repositório oficial dos <strong className="text-white/80">templates</strong> do sistema.
      ...
      <div className="overflow-hidden rounded-2xl border border-[var(--border-default)] bg-[var(--surface-primary)] backdrop-blur-sm">
        <table className="w-full text-sm">
          ...
          <tbody>
            {lista.carregando ? (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-xs text-white/40">Carregando…</td></tr>
            ) : modelos.length === 0 ? (
              ...
              <tr key={m.id} className="border-b border-[var(--border-subtle)] last:border-0">
                ...
                <td className="px-4 py-2.5 text-white/70">
                  {m.versaoPublicada ? `v${m.versaoPublicada.numero}` : <span className="text-amber-300/80">nenhuma</span>}
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-[var(--overlay-modal)] p-4 backdrop-blur-sm" onClick={fechar}>
      <div className="my-8 w-full max-w-5xl rounded-2xl border border-[var(--border-default)] bg-zinc-900/95 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-4">
          {detalhe.carregando ? (
            <div className="py-8 text-center text-xs text-white/40">Carregando…</div>
          ) : !modelo?.versoes?.length ? (
            <div className="py-8 text-center text-xs text-white/40">Nenhuma versão enviada.</div>
          ) : (
            <div className="space-y-3">
              {modelo.versoes.map((v) => (
                <div key={v.id} className="rounded-xl border border-[var(--border-default)] bg-white/[0.03] p-4">
                  ...
                  <dl className="mt-3 grid gap-x-6 gap-y-1 text-xs text-white/50 sm:grid-cols-2 lg:grid-cols-4">
                    <div><dt className="inline text-white/40">Arquivo: </dt><dd className="inline text-white/70">{v.arquivoNome}</dd></div>
                    <div><dt className="inline text-white/40">Criada: </dt><dd className="inline text-white/70">{new Date(v.criadoEm).toLocaleString("pt-BR")}</dd></div>
                    <div><dt className="inline text-white/40">Autor: </dt><dd className="inline text-white/70">{v.criadoPor?.nome ?? "—"}</dd></div>
                    <div><dt className="inline text-white/40">Publicada: </dt><dd className="inline text-white/70">{v.publicadoEm ? `${new Date(v.publicadoEm).toLocaleString("pt-BR")} · ${v.publicadoPor?.nome ?? "—"}` : "—"}</dd></div>
                    <div className="sm:col-span-2 lg:col-span-4">
                      <dt className="inline text-white/40">Checksum: </dt>
                      <dd className="inline break-all font-mono text-[10px] text-white/60">{v.checksum}</dd>
                    </div>
                  </dl>
                    ...
                    <div className="mt-3 rounded-lg border border-[var(--border-default)] bg-black/30 p-3">
                      ...
                      <ul className="mt-2 space-y-1 text-[11px] text-white/60">
                        {validacao.resultado.achados.map((a, i) => (
                          <li key={i}>
                            <span className={a.severidade === "erro" ? "text-red-300" : "text-amber-300"}>■</span>{" "}
                            {a.mensagem} {a.detalhe && <span className="text-white/40">{a.detalhe}</span>}
                          </li>
                        ))}
                        {validacao.resultado.achados.length === 0 && <li className="text-white/40">Nenhum achado.</li>}
                      </ul>

                      {validacao.resultado.naoDeclarados.length > 0 && (
                        <div className="mt-3">
                          <p className="text-[11px] text-white/50">
                            Confirme quais números são <strong className="text-white/70">dados fixos do outorgado</strong> e
                          ...
                          <div className="mt-1 space-y-1">
                            {validacao.resultado.naoDeclarados.map((l) => {
                              ...
                              return (
                                <label key={l.digitos} className="flex items-center gap-2 text-[11px] text-white/70">
                                  <input
                                    type="checkbox"
                                    checked={marcado}
                                    onChange={(e) =>
                                  ...
                                  />
                                  <span className="uppercase text-white/40">{l.tipo}</span> {l.valor}
                                </label>
                              )
                            })}
                          </div>
