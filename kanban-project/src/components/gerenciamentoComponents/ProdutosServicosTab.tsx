...
export default function ProdutosServicosTab() {
  ...
  const buscar = useCallback(async (sinal?: AbortSignal) => {
      const d = await jsonFetch('/api/gerenciamento/produtos-servicos', { cache: 'no-store', signal: sinal })
    return d
  }, [])
  ...
  useEffect(() => {
    ...
    buscar(ac.signal)
      ...
      .catch((e: any) => { if (!ac.signal.aborted) setErroLista(e.message || 'Não foi possível carregar os serviços.') })
  ...
  const carregar = useCallback(async () => {
    ...
    catch (e: any) { setErroLista(e.message || 'Não foi possível carregar os serviços.') }
