...
export default function CarteirasTab() {
  ...
  const buscar = useCallback(async (sinal?: AbortSignal) => {
      const [dCart, dContas] = await Promise.all([
        jsonFetch('/api/gerenciamento/carteiras', { cache: 'no-store', signal: sinal }),
        jsonFetch('/api/gerenciamento/contas', { cache: 'no-store', signal: sinal }).catch(() => ({ contas: [] })),
      ])
    return { dCart, dContas }
  }, [])
  // APLICAÇÃO: ponto único de escrita do estado da carga.
  const aplicar = useCallback(({ dCart, dContas }: any) => {
  ...
  useEffect(() => {
    ...
    buscar(ac.signal)
      ...
      .catch((e: any) => { if (!ac.signal.aborted) setErroLista(e.message || 'Não foi possível carregar as carteiras.') })
  ...
  const carregar = useCallback(async () => {
    ...
    catch (e: any) { setErroLista(e.message || 'Não foi possível carregar as carteiras.') }
