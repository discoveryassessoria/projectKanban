  ...
  return (
    <div className="space-y-5">
      ...
      <div className="relative">
        ...
        <input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar forma de pagamento…" className="w-full rounded-lg border border-white/10 bg-black/30 py-2 pl-9 pr-3 text-sm text-white placeholder:text-white/30 outline-none focus:border-white/25" />
      </div>
            ...
            <div key={x.id} className={`${GLASS} flex items-center gap-4 p-4 transition hover:border-white/20`}>
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {x.type && <span className="shrink-0 rounded-md border border-white/15 px-2 py-0.5 text-[11px] text-white/60">{TIPOS_FORMA_LABEL[x.type] || x.type}</span>}
              ...
              <div className="flex shrink-0 items-center gap-2">
                <button onClick={() => abrirEditar(x)} className="inline-flex items-center gap-1 rounded-md border border-white/10 px-2.5 py-1 text-xs text-white/70 transition hover:bg-white/10 hover:text-white"><Pencil className="h-3 w-3" /> Editar</button>
...
function FormaPanel({ f, set, editId, moedas, carteiras, contas, salvando, erro, onClose, onSalvar }: {
  ...
  const input = 'w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder:text-white/30 outline-none focus:border-white/30'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="max-h-[92vh] w-full max-w-3xl overflow-auto rounded-2xl border border-white/10 bg-zinc-900/95 text-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="sticky bottom-0 flex items-center justify-end gap-2 border-t border-white/10 bg-zinc-900/95 px-6 py-4">
...
function Select({ value, onChange, options }: { value: string; onChange: (v: string) => void; options: [string, string][] }) {
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white outline-none focus:border-white/30">
