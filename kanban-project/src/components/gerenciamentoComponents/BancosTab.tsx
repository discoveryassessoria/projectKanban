...
export default function BancosTab() {
  ...
  const buscar = useCallback(async (sinal?: AbortSignal) => {
      const data = await jsonFetch('/api/gerenciamento/bancos', { cache: 'no-store', signal: sinal })
    return data
  }, [])
  ...
  useEffect(() => {
    ...
    buscar(ac.signal)
      ...
      .catch((e: any) => { if (!ac.signal.aborted) setErroLista(e.message || 'Não foi possível carregar os bancos.') })
  ...
  const carregar = useCallback(async () => {
    ...
    catch (e: any) { setErroLista(e.message || 'Não foi possível carregar os bancos.') }
