  ...
  return (
    <div className="bg-[var(--surface-primary)] backdrop-blur-sm border border-[var(--border-default)] rounded-xl p-4">
      <div className="text-[11px] text-[var(--text-secondary)] font-bold uppercase tracking-wider mb-2 flex items-center gap-1.5">
        ...
        {abertas.length > 0 && <span className="text-[10px] bg-amber-50 text-amber-700 px-1.5 rounded-full">{abertas.length}</span>}
      </div>
      {abertas.length === 0 ? (
        <p className="text-xs text-[var(--text-muted)] flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-green-700" /> Nenhuma pendência aberta.</p>
      ) : (
        <ul className="space-y-2">
          {abertas.slice(0, compact ? 3 : 20).map((p) => (
            <li key={p.id} className="rounded-lg border border-amber-200 bg-amber-500/[0.06] p-2">
              <div className="flex items-center justify-between gap-2">
                <span className="text-[11px] font-semibold text-amber-700">{p.motivo}{p.natureza ? ` · ${p.natureza}` : ""}</span>
              ...
              <div className="text-[10px] text-blue-700 mt-1">→ {p.acaoSugerida}</div>
            </li>
          ))}
        </ul>
      )}
