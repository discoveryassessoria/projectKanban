...
export default function DuplicarReceitaModal({ receitaRef, onClose, onDone }: {
  receitaRef: string; onClose: () => void; onDone?: (obrigacaoIdNovo: number) => void
}) {
    ...
    <div className="fixed inset-0 flex items-start justify-center overflow-y-auto bg-[var(--app-overlay)] p-4 sm:items-center" style={{ zIndex: LAYER.aboveProcess }} onClick={() => !enviando && onClose()}>
      <div className="w-full max-w-md rounded-[var(--radius-lg)] border border-[var(--border-default)] bg-[var(--surface-overlay)] shadow-[var(--shadow-surface)]" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-[var(--border-default)] px-5 py-4">
          <h2 className="flex items-center gap-2 text-base font-semibold text-[var(--text-primary)]"><Copy className="h-4 w-4 text-[var(--info)]" /> Duplicar Receita</h2>
        ...
        <div className="space-y-3 px-5 py-4">
          <p className="flex items-start gap-1.5 rounded-[var(--radius-sm)] border p-3 text-xs text-[var(--text-secondary)]" style={{ borderColor: "color-mix(in srgb, var(--info) 25%, transparent)", background: "color-mix(in srgb, var(--info) 5%, transparent)" }}><InfoIcon className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[var(--info)]" /> Cria uma nova Receita com o mesmo item, valor-base, moeda, câmbio, distribuição e participantes. Pagamentos, cobranças e histórico da origem NÃO são copiados. A nova Receita abre em seguida.</p>
          <div>
