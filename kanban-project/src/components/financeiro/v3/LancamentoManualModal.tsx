...
export function LancamentoManualModal({ natureza, processoId, onClose, onCriado }: { natureza: Natureza; processoId: number; onClose: () => void; onCriado: () => void }) {
  ...
  const inputCls = "mt-1 w-full rounded-lg border border-neutral-800 bg-[#0a0b0d] px-3 py-2 text-sm text-neutral-100 outline-none placeholder:text-neutral-600"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4" onClick={onClose}>
      <div className="my-6 w-full max-w-2xl rounded-xl border border-neutral-800 bg-[#0f1114] p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          ...
          <label className={`${labelCls} col-span-2`}>Descrição {receita ? "" : ""}<span className="text-neutral-600"> (complementar, opcional)</span>
          ...
          {/* Fase / Vencimento */}
          <label className={labelCls}>Fase <span className="text-neutral-600">(opcional)</span>
          ...
          <label className={labelCls}>Vencimento <span className="text-neutral-600">(opcional)</span>
        ...
        {/* ── Vínculo / Rateio ── */}
        <div className="mt-4 rounded-lg border border-neutral-800 bg-[#0a0b0d] p-3">
          <div className="mb-2 text-xs font-medium text-neutral-400">Vínculo</div>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setVinculo("processo")} className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs ${vinculo === "processo" ? "border-amber-500/50 bg-amber-500/10 text-amber-300" : "border-neutral-700 text-neutral-300"}`}><Building2 className="h-3.5 w-3.5" /> Processo inteiro</button>
            <button onClick={() => setVinculo("requerentes")} className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs ${vinculo === "requerentes" ? "border-amber-500/50 bg-amber-500/10 text-amber-300" : "border-neutral-700 text-neutral-300"}`}><Users className="h-3.5 w-3.5" /> Requerente(s)</button>
          </div>

          {vinculo === "requerentes" && (
            <div className="mt-3">
              {requerentes.length === 0 ? <div className="text-xs text-neutral-500">Este processo não tem requerentes cadastrados.</div> : (
                <div className="flex flex-wrap gap-2">
                  {requerentes.map((r) => (
                    <button key={r.id} onClick={() => toggleReq(r.id)} title={r.personId == null ? "Sem identidade vinculada — não entra em rateio" : ""} className={`inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1 text-xs ${selReq.has(r.id) ? "border-violet-500/50 bg-violet-500/10 text-violet-200" : "border-neutral-700 text-neutral-300"} ${r.personId == null ? "opacity-60" : ""}`}><User className="h-3 w-3" /> {r.nome}</button>
                  ))}
                </div>
              )}
              {reqSelecionados.length > 1 && (
                <div className="mt-3">
                  <div className="mb-1 text-xs text-neutral-400">Rateio</div>
                  <div className="flex gap-2">
                    {(["IGUAL", "PERCENTUAL", "VALOR"] as ModoRateio[]).map((m) => (
                      <button key={m} onClick={() => setModoRateio(m)} className={`rounded-lg border px-2.5 py-1 text-xs ${modoRateio === m ? "border-amber-500/50 bg-amber-500/10 text-amber-300" : "border-neutral-700 text-neutral-300"}`}>{m === "IGUAL" ? "Igual" : m === "PERCENTUAL" ? "Percentual" : "Valor"}</button>
                    ))}
                  </div>
                  {modoRateio !== "IGUAL" && (
                    <div className="mt-2 space-y-1.5">
                      {reqSelecionados.map((r) => (
                        <div key={r.id} className="flex items-center gap-2">
                          <span className="w-40 truncate text-xs text-neutral-300">{r.nome}</span>
                          <input value={rateioVal[r.id] ?? ""} onChange={(e) => setRateioVal((s) => ({ ...s, [r.id]: e.target.value }))} inputMode="decimal" placeholder={modoRateio === "PERCENTUAL" ? "%" : moeda} className="w-28 rounded-lg border border-neutral-800 bg-[#0a0b0d] px-2 py-1 text-xs text-neutral-100 outline-none" />
                        </div>
                      ))}
                    </div>
                  )}
        ...
        {/* ── Prévia ── */}
        <div className="mt-4 rounded-lg border border-neutral-800 bg-[#0a0b0d] p-3 text-sm">
          <div className="flex justify-between text-neutral-400"><span>Subtotal</span><span className="text-neutral-200">{fmt(subtotal, moeda)}</span></div>
          {desc > 0 && <div className="flex justify-between text-neutral-400"><span>Desconto</span><span className="text-red-300">− {fmt(desc, moeda)}</span></div>}
          {!receita && acr > 0 && <div className="flex justify-between text-neutral-400"><span>Acréscimos</span><span className="text-neutral-200">+ {fmt(acr, moeda)}</span></div>}
          <div className="mt-1 flex justify-between border-t border-neutral-800 pt-1 font-semibold text-white"><span>Total</span><span>{fmt(total, moeda)}</span></div>
          {distribuicao.length > 0 && (
            <div className="mt-2 border-t border-neutral-800 pt-2">
              <div className="mb-1 text-xs text-neutral-500">Distribuição por requerente</div>
              {distribuicao.map((d, i) => <div key={i} className="flex justify-between text-xs text-neutral-300"><span>{d.nome} <span className="text-neutral-500">({d.pct.toFixed(1)}%)</span></span><span>{fmt(d.valor, moeda)}</span></div>)}
              {Math.abs(somaRateio - total) > 0.01 && <div className="mt-1 text-xs text-amber-400">⚠ Soma do rateio ({fmt(somaRateio, moeda)}) difere do total ({fmt(total, moeda)}).</div>}
            </div>
          )}
        </div>
