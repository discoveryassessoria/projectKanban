    ...
    <div className="text-[var(--text-secondary)]">
      <div className="mb-5 flex flex-wrap gap-6 border-b border-[var(--border-default)]">
        {SUBTABS.map(([id, label]) => (
          <button key={id} onClick={() => setT(id)} className={`-mb-px border-b-2 px-1 pb-3 pt-1 text-sm ${t === id ? "border-[var(--accent-primary)] font-medium text-[var(--accent-text)]" : "border-transparent text-[var(--text-secondary)] hover:text-[var(--text-secondary)]"}`}>{label}</button>
        ))}
      </div>
      ...
      <div className="mt-5 rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)]">
        <div className="flex items-center gap-6 border-b border-[var(--border-default)] px-5 pt-4">
          {([["todos", "Todos"], ["pagos", "Pagos"], ["apagar", "A Pagar"]] as const).map(([id, label]) => (
            <button key={id} onClick={() => { setSub(id); setPage(1) }} className={`-mb-px border-b-2 pb-3 text-sm ${sub === id ? "border-[var(--accent-primary)] font-medium text-[var(--accent-text)]" : "border-transparent text-[var(--text-secondary)] hover:text-[var(--text-secondary)]"}`}>{label}</button>
          ))}
        </div>
