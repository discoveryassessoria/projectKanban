      ...
      <div className="mx-auto max-w-[1180px] px-4 py-6 sm:px-6">
        ...
        <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={salvar} disabled={!valido || salvando || ok} title={valido ? "" : bloqueado ? previa?.bloqueios[0] : !temMudanca ? "Nenhuma alteração" : ""} className="inline-flex items-center gap-2 rounded-[var(--radius-sm)] bg-[var(--info)] px-4 py-2 text-sm font-semibold text-[var(--accent-ink)] hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50">
        ...
        {loading ? (
          <div className="flex h-64 items-center justify-center text-[var(--text-muted)]"><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Carregando {ehCusto ? "custo" : "receita"}…</div>
        ) : erro ? (
          ...
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_340px]">
            ...
            <div className="space-y-5">
              ...
              <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                <h2 className="mb-4 flex items-center gap-1.5 text-sm font-semibold text-[var(--text-secondary)]"><Receipt className="h-4 w-4" /> Dados {ehCusto ? "do custo" : "da receita"}</h2>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div className="sm:col-span-2">
                    ...
                    <p className="mt-1 text-[11px] text-[var(--text-muted)]">Propaga a todos os participantes preservando o sufixo de cada um.</p>
                  </div>
                  <div className="sm:col-span-2">
              ...
              {/* Valor-base + câmbio */}
              <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                <h2 className="mb-4 flex items-center gap-1.5 text-sm font-semibold text-[var(--text-secondary)]"><Coins className="h-4 w-4" /> Valor-base e câmbio</h2>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {(["VARIAVEL", "FIXO"] as FxRule[]).map((r) => (
                        <button key={r} onClick={() => setFxRule(r)} disabled={moeda === "BRL"} className={`flex-1 rounded-[var(--radius-sm)] border px-3 py-2 text-xs font-medium disabled:opacity-40 ${fxRule === r ? "border-[var(--info)] bg-[var(--surface-hover)] text-[var(--text-primary)]" : "border-[var(--border-default)] text-[var(--text-secondary)] hover:bg-[var(--surface-hover)]"}`}>{r === "FIXO" ? "Fixo" : "Variável"}</button>
                      ))}
                    </div>
                  </div>
                  <div>
                  ...
                  {fxRule === "FIXO" && (
                    <>
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-[11px] text-[var(--text-muted)]">Se informado, prevalece sobre a cotação. Rateado entre os participantes.</p>
                      </div>
                    </>
                  )}
                </div>
                <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-1 rounded-[var(--radius-sm)] bg-[var(--surface-secondary)] px-4 py-3 text-sm">
                  <div><span className="text-[var(--text-muted)]">Valor-base:</span> <span className="font-medium text-[var(--text-primary)]">{money(cent(num(valorBase)), moeda)}</span></div>
                  <div><span className="text-[var(--text-muted)]">Cotação efetiva:</span> <span className="font-medium text-[var(--text-primary)]">{cotacaoEfetiva != null ? cotacaoEfetiva.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 4 }) : "—"}</span></div>
                  <div><span className="text-[var(--text-muted)]">Total BRL:</span> <span className="font-medium text-[var(--info)]">{brl(brlTotal)}</span></div>
                  {previewing && <span className="inline-flex items-center gap-1 text-xs text-[var(--text-muted)]"><Loader2 className="h-3 w-3 animate-spin" /> calculando…</span>}
                </div>
              </section>
              ...
              {mudouFinanceiro && previa && (
                <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                  <h2 className="mb-1 flex items-center gap-1.5 text-sm font-semibold text-[var(--text-secondary)]"><ArrowRight className="h-4 w-4" /> Impacto da alteração</h2>
                  <p className="mb-3 text-xs text-[var(--text-muted)]">Prévia calculada pelo motor. Nada é gravado até você salvar.</p>

                  <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                    <div className="rounded-[var(--radius-sm)] bg-[var(--surface-secondary)] p-3"><div className="text-[11px] uppercase tracking-wide text-[var(--text-muted)]">Valor-base</div><div className="text-sm text-[var(--text-primary)]">{money(previa.valorBaseTotalAntigo, rec.moedaBase)} → <span className="font-semibold text-[var(--text-primary)]">{money(previa.valorBaseTotalNovo, previa.moedaBase)}</span></div></div>
                    <div className="rounded-[var(--radius-sm)] bg-[var(--surface-secondary)] p-3"><div className="text-[11px] uppercase tracking-wide text-[var(--text-muted)]">Total BRL</div><div className="text-sm text-[var(--text-primary)]">{brl(previa.valorContratadoBrlAntigo)} → <span className="font-semibold text-[var(--info)]">{brl(previa.valorContratadoBrlNovo)}</span></div></div>
                    <div className="rounded-[var(--radius-sm)] bg-[var(--surface-secondary)] p-3"><div className="text-[11px] uppercase tracking-wide text-[var(--text-muted)]">Já recebido (BRL)</div><div className="text-sm font-semibold text-[var(--success)]">{brl(previa.recebidoTotalBrl)}</div></div>
                  </div>

                  {previa.temPagamentoConfirmado && (
                    <div className="mb-3 flex items-start gap-2 rounded-[var(--radius-sm)] border p-3 text-xs text-[var(--info)]" style={{ borderColor: "color-mix(in srgb, var(--info) 25%, transparent)", background: "color-mix(in srgb, var(--info) 10%, transparent)" }}>
                      ...
                      <table className="w-full min-w-[520px] text-sm">
                        ...
                        <tbody>
                          {previa.cobrancasAfetadas.map((c) => (
                            <tr key={c.parcelaId} className="border-t border-[var(--border-default)]">
                              <td className="px-2 py-2 text-[var(--text-secondary)]">Parcela {c.numero}</td>
                              <td className="px-2 py-2 text-[var(--text-secondary)]">{dataBR(c.vencimento)}</td>
                              <td className="px-2 py-2 text-right text-[var(--text-muted)]">{money(c.valorBaseAntigo, previa.moedaBase)}</td>
                              <td className="px-2 py-2 text-right font-medium text-[var(--text-primary)]">{money(c.valorBaseNovo, previa.moedaBase)}</td>
                              <td className="px-2 py-2 text-right text-[var(--info)]">{brl(c.valorBrlNovo)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                      <p className="mt-2 text-[11px] text-[var(--text-muted)]">{previa.cobrancasAfetadas.length} cobrança(s) em aberto seriam ajustadas com a estratégia “Atualizar abertas”.</p>
                    </div>
                  ) : (
                    <div className="flex items-start gap-2 rounded-[var(--radius-sm)] border border-[var(--border-default)] bg-[var(--surface-secondary)] p-3 text-xs text-[var(--text-muted)]">
                      <InfoIcon className="mt-0.5 h-4 w-4 shrink-0 text-[var(--info)]" /><span>Sem cobranças em aberto para reescalar. A diferença é registrada como ajuste no razão (Ledger).</span>
                    </div>
                  )}

                  {previa.bloqueios.length > 0 && (
                    <div className="mt-3 rounded-[var(--radius-sm)] border p-3 text-xs text-[var(--danger)]" style={{ borderColor: "color-mix(in srgb, var(--danger) 30%, transparent)", background: "color-mix(in srgb, var(--danger) 10%, transparent)" }}>
            ...
            <div className="space-y-4 lg:sticky lg:top-4 lg:self-start">
              <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                <h2 className="mb-3 text-sm font-semibold text-[var(--text-secondary)]">Situação atual</h2>
                <dl className="space-y-2 text-sm">
                  <div className="flex justify-between"><dt className="text-[var(--text-muted)]">Código</dt><dd className="text-[var(--text-primary)]">{rec.codigo ?? "—"}</dd></div>
                  <div className="flex justify-between"><dt className="text-[var(--text-muted)]">Participantes</dt><dd className="text-[var(--text-primary)]">{rec.participantes.length}</dd></div>
                  <div className="flex justify-between"><dt className="text-[var(--text-muted)]">Valor contratado (BRL)</dt><dd className="text-[var(--text-primary)]">{brl(rec.valorContratadoBrlTotal)}</dd></div>
                  <div className="flex justify-between"><dt className="text-[var(--text-muted)]">Recebido (BRL)</dt><dd className="text-[var(--success)]">{brl(rec.recebidoTotalBrl)}</dd></div>
                  <div className="flex justify-between"><dt className="text-[var(--text-muted)]">Cobranças abertas</dt><dd className="text-[var(--text-primary)]">{rec.cobrancasAbertas}</dd></div>
                  <div className="flex items-center justify-between pt-1"><dt className="text-[var(--text-muted)]">Pagamento confirmado</dt><dd><span className={`rounded-[var(--radius-sm)] px-2 py-0.5 text-[11px] font-semibold ${rec.temPagamentoConfirmado ? "text-[var(--info)]" : "bg-[var(--surface-hover)] text-[var(--text-muted)]"}`} style={rec.temPagamentoConfirmado ? { background: "color-mix(in srgb, var(--info) 15%, transparent)" } : undefined}>{rec.temPagamentoConfirmado ? "Sim" : "Não"}</span></dd></div>
                </dl>
              </section>
              ...
              {mudouFinanceiro && (
                <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                  ...
                  <p className="mb-3 text-[11px] text-[var(--text-muted)]">Como aplicar a mudança de valor/câmbio às cobranças em aberto.</p>
                  <div className="space-y-2">
                    {([["ATUALIZAR_ABERTAS", "Atualizar cobranças em aberto", "Reescala as parcelas pendentes ao novo valor."], ["AJUSTE_COMPENSATORIO", "Ajuste compensatório", "Preserva as parcelas e lança a diferença no razão."]] as const).map(([v, lb, desc]) => (
                      <label key={v} className={`flex cursor-pointer items-start gap-2 rounded-[var(--radius-sm)] border p-2.5 ${estrategia === v ? "border-[var(--info)] bg-[var(--surface-hover)]" : "border-[var(--border-default)]"}`}>
              ...
              <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                <h2 className="mb-2 text-sm font-semibold text-[var(--text-secondary)]">Justificativa (auditoria)</h2>
                <textarea value={justificativa} onChange={(e) => setJustificativa(e.target.value.slice(0, 300))} rows={3} placeholder="Motivo desta edição…" className="w-full resize-none rounded-[var(--radius-sm)] border border-[var(--border-default)] bg-[var(--surface-input)] px-3 py-2 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--info)]" />
                <p className="mt-2 text-[11px] text-[var(--text-muted)]">Registrada em cada obrigação afetada (estado anterior → novo).</p>
              </section>

              {erroSave && <div className="rounded-[var(--radius-sm)] border p-3 text-xs text-[var(--danger)]" style={{ borderColor: "color-mix(in srgb, var(--danger) 30%, transparent)", background: "color-mix(in srgb, var(--danger) 10%, transparent)" }}>{erroSave}</div>}
              {!temMudanca && <div className="rounded-[var(--radius-sm)] border border-[var(--border-default)] bg-[var(--surface-secondary)] p-3 text-xs text-[var(--text-muted)]">Nenhuma alteração ainda. Edite um campo para habilitar o salvamento.</div>}
              {ok && <div className="rounded-[var(--radius-sm)] border p-3 text-xs text-[var(--success)]" style={{ borderColor: "color-mix(in srgb, var(--success) 30%, transparent)", background: "color-mix(in srgb, var(--success) 10%, transparent)" }}>{ehCusto ? "Custo atualizado." : "Receita atualizada."}</div>}
            </div>
          </div>
        ) : null}
      </div>
