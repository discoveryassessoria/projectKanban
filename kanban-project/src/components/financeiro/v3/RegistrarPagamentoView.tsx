...
const statusTone = (s?: string | null): string => {
  ...
  if (S.includes("EXCED")) return "var(--info)"
...
export default function RegistrarPagamentoView({ obrigacaoId, receitaRef, escopo, onTrocarEscopo, onClose, onDone }: {
  ...
  const idemKey = () => (idemRef.current ??= `ui-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`)
    ...
    if (ehGeral) {
      try {
        ...
        const r = await fetch(`/api/financeiro/v3/receita/${receitaRef}/registrar-pagamento-geral`, {
          ...
          body: JSON.stringify({ alocacoes: alocacoesGeral.map((a) => ({ obrigacaoId: a.obrigacaoId, valor: a.valor })), formas, ajustes: { desconto, juros, multa, acrescimo, creditoUtilizado }, pagador: pagadorG, observacao: observacao || "[Pagamento geral da Receita]", idempotencyKey: idemKey() }),
        }).then((x) => x.json())
  ...
  return createPortal((
    <div className="fixed inset-0 overflow-y-auto bg-[var(--surface-overlay)]" style={{ zIndex: LAYER.aboveProcess }}>
      <div className="mx-auto max-w-[1180px] px-4 py-6 sm:px-6">
        ...
        <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="mb-1 flex flex-wrap items-center gap-1.5 text-xs text-[var(--text-muted)]">
              <span>Financeiro</span><span>›</span><span>Receitas</span><span>›</span><span className="text-[var(--text-secondary)]">{codigo}</span><span>›</span><span>Cobranças</span><span>›</span><span className="text-[var(--text-secondary)]">Registrar pagamento</span>
            </div>
            <h1 className="text-2xl font-semibold text-[var(--text-primary)]">Registrar pagamento</h1>
            <p className="mt-0.5 text-sm text-[var(--text-muted)]">Registre um ou mais recebimentos para esta cobrança.</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={submit} disabled={!valido || enviando || ok} title={valido ? "" : pendencias[0]} className="inline-flex items-center gap-2 rounded-[var(--radius-sm)] bg-[var(--accent-primary)] px-4 py-2 text-sm font-semibold text-[var(--accent-ink)] hover:bg-[var(--accent-hover)] disabled:cursor-not-allowed disabled:opacity-50">
        ...
        {escopo && (
          <div className="mb-4 flex flex-wrap items-center justify-between gap-2 rounded-[var(--radius-md)] border px-4 py-2.5" style={{ borderColor: "color-mix(in srgb, var(--accent-primary) 25%, transparent)", background: "color-mix(in srgb, var(--accent-primary) 5%, transparent)" }}>
            <span className="text-sm text-[var(--text-secondary)]"><span className="text-[var(--text-muted)]">Escopo:</span> <span className="font-medium text-[var(--accent-hover)]">{escopo.tag}</span></span>
            {onTrocarEscopo && <button onClick={onTrocarEscopo} className="rounded-[var(--radius-sm)] border border-[var(--border-default)] px-3 py-1 text-xs font-medium text-[var(--text-secondary)] hover:bg-[var(--surface-hover)]">Trocar escopo</button>}
          </div>
        )}

        {loading ? (
          <div className="flex h-64 items-center justify-center text-[var(--text-muted)]"><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Carregando cobrança…</div>
        ) : erro ? (
          <div className="rounded-[var(--radius-md)] border p-4 text-sm text-[var(--danger)]" style={{ borderColor: "color-mix(in srgb, var(--danger) 30%, transparent)", background: "color-mix(in srgb, var(--danger) 10%, transparent)" }}>{erro}</div>
        ) : (
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_320px]">
            ...
            <div className="space-y-5">
              ...
              <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                <h2 className="mb-4 text-sm font-semibold text-[var(--text-secondary)]">Resumo da Cobrança</h2>
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                  <Campo label="Receita"><div className="text-sm font-medium text-[var(--text-primary)]">{det?.descricao ?? "—"}</div><span className="mt-1 inline-block rounded bg-[var(--surface-hover)] px-1.5 py-0.5 text-[10px] text-[var(--text-secondary)]">{codigo}</span></Campo>
                  <Campo label="Participante Financeiro"><div className="flex items-center gap-2"><span className="grid h-7 w-7 place-items-center rounded-full bg-[var(--surface-hover)] text-[11px] font-semibold text-[var(--text-secondary)]">{iniciais(det?.responsavel?.nome)}</span><span className="truncate text-sm text-[var(--text-primary)]">{det?.responsavel?.nome ?? "—"}</span></div></Campo>
                  <Campo label="Vencimento"><div className="text-sm text-[var(--text-primary)]">{dataBR(det?.proximoVencimento)}</div></Campo>
                  <Campo label="Status"><span className="inline-block rounded-[var(--radius-sm)] px-2 py-0.5 text-[11px] font-semibold" style={statusStyle(det?.statusLabel)}>{det?.statusLabel ?? "—"}</span></Campo>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-4 border-t border-[var(--border-default)] pt-4 sm:grid-cols-3 lg:grid-cols-6">
                  <Campo label="Valor Original (BRL)"><V>{brl(det?.valorContratadoBrl ?? 0)}</V></Campo>
                  <Campo label="Valor Recebido (BRL)"><V cls="text-[var(--success)]">{brl(det?.recebidoBrl ?? 0)}</V></Campo>
                  <Campo label="Saldo Atual (BRL)"><V cls="text-[var(--danger)]">{brl(saldoCobranca)}</V></Campo>
                  ...
                  <Campo label="Câmbio Aplicado"><V>{det?.cotacao ? Number(det.cotacao).toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 4 }) : "—"}</V><span className="text-[10px] text-[var(--text-muted)]">{det?.tipoCambio ?? ""}</span></Campo>
                </div>
              </section>
              ...
              <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                <div className="mb-3 flex items-center justify-between">
                  <div><h2 className="text-sm font-semibold text-[var(--text-secondary)]">1. Composição do Pagamento</h2><p className="text-xs text-[var(--text-muted)]">Informe uma ou mais formas de pagamento utilizadas neste recebimento.</p></div>
                  <button onClick={addLinha} className="inline-flex items-center gap-1.5 rounded-[var(--radius-sm)] border border-[var(--border-default)] px-3 py-1.5 text-xs font-medium text-[var(--text-secondary)] hover:bg-[var(--surface-hover)]"><Plus className="h-3.5 w-3.5" /> Adicionar forma de pagamento</button>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[820px] text-sm">
                    ...
                    <tbody>
                      {linhas.map((l) => (
                        <tr key={l._id} className="border-t border-[var(--border-default)]">
                          <td className="px-2 py-1.5 align-top">
                            ...
                            {ehCartao(l) && (
                              <div className="mt-1.5 space-y-1.5 rounded-[var(--radius-sm)] border border-[var(--border-default)] bg-[var(--surface-secondary)] p-2">
                ...
                <div className="mt-3 flex items-center justify-between rounded-[var(--radius-sm)] bg-[var(--surface-secondary)] px-4 py-2.5"><span className="flex items-center gap-1.5 text-xs text-[var(--text-muted)]"><InfoIcon className="h-3.5 w-3.5" /> Total informado (BRL)</span><span className="text-base font-semibold text-[var(--text-primary)]">{brl(totalInformado)}</span></div>
              </section>

              {/* 2. Pagador */}
              <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                <h2 className="text-sm font-semibold text-[var(--text-secondary)]">2. Pagador</h2>
                <p className="mb-3 text-xs text-[var(--text-muted)]">Selecione quem está realizando este pagamento.</p>
                <div className="mb-4 flex flex-wrap gap-2">
                  {([["REQUERENTE", "Participante Financeiro", UsersRound], ["EMPRESA", "Empresa", Building2], ["TERCEIRO", "Terceiro", UserIcon], ["EXTERNO", "Outro", UserIcon]] as const).map(([v, lb, Ic]) => (
                    <button key={v} onClick={() => setPagadorTipo(v)} className={`inline-flex items-center gap-1.5 rounded-[var(--radius-sm)] border px-3 py-1.5 text-xs font-medium ${pagadorTipo === v ? "" : "border-[var(--border-default)] text-[var(--text-secondary)] hover:bg-[var(--surface-hover)]"}`} style={pagadorTipo === v ? { borderColor: "color-mix(in srgb, var(--accent-primary) 60%, transparent)", background: "color-mix(in srgb, var(--accent-primary) 10%, transparent)", color: "var(--accent-hover)" } : undefined}><Ic className="h-3.5 w-3.5" /> {lb}</button>
                  ))}
                </div>
                {pagadorTipo === "REQUERENTE" ? (
                  ...
                  <p className="text-sm text-[var(--text-muted)]">Pagamento realizado por {pagadorTipo === "EMPRESA" ? "empresa" : "terceiro"}.</p>
                )}
              </section>

              {/* 3. Ajustes Financeiros */}
              <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                <h2 className="text-sm font-semibold text-[var(--text-secondary)]">3. Ajustes Financeiros</h2>
                <p className="mb-3 text-xs text-[var(--text-muted)]">Informe ajustes que impactam o valor final deste recebimento.</p>
                ...
                {situacao === "PARCIAL" && (
                  <div className="mt-3 rounded-[var(--radius-sm)] border p-3" style={{ borderColor: "color-mix(in srgb, var(--info) 25%, transparent)", background: "color-mix(in srgb, var(--info) 5%, transparent)" }}>
                    <p className="mb-2 flex items-center gap-1.5 text-sm text-[var(--info)]"><InfoIcon className="h-4 w-4" /> Pagamento parcial identificado. Saldo restante: {brl(saldoRestante)}. Como deseja tratar este saldo?</p>
                ...
                {situacao === "EXCEDENTE" && (
                  <div className="mt-3 rounded-[var(--radius-sm)] border p-3" style={{ borderColor: "color-mix(in srgb, var(--accent-primary) 25%, transparent)", background: "color-mix(in srgb, var(--accent-primary) 5%, transparent)" }}>
                    <p className="mb-2 flex items-center gap-1.5 text-sm text-[var(--accent-hover)]"><AlertTriangle className="h-4 w-4" /> Foi identificado um valor excedente de {brl(excedente)}.</p>
              ...
              <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                <h2 className="text-sm font-semibold text-[var(--text-secondary)]">4. Aplicação do Pagamento</h2>
                <p className="mb-3 text-xs text-[var(--text-muted)]">{ehGeral ? "Pagamento geral: aloque o valor entre os participantes (nenhum é assumido)." : "Defina como este valor será aplicado."}</p>
                {ehGeral ? (
                  <div className="space-y-3">
                    <div className="flex gap-2">{(["AUTOMATICA", "MANUAL"] as const).map((mm) => (<button key={mm} onClick={() => setAlocGeralModo(mm)} className={`rounded-[var(--radius-sm)] border px-3 py-1.5 text-xs font-medium ${alocGeralModo === mm ? "" : "border-[var(--border-default)] text-[var(--text-secondary)] hover:bg-[var(--surface-hover)]"}`} style={alocGeralModo === mm ? { borderColor: "color-mix(in srgb, var(--accent-primary) 60%, transparent)", background: "color-mix(in srgb, var(--accent-primary) 10%, transparent)", color: "var(--accent-hover)" } : undefined}>{mm === "AUTOMATICA" ? "Distribuição automática" : "Seleção manual"}</button>))}</div>
                    <div className="space-y-1.5 rounded-[var(--radius-sm)] bg-[var(--surface-secondary)] p-3">
                      {alocacoesGeral.map((a) => (<div key={a.obrigacaoId} className="flex items-center justify-between gap-2 text-sm"><span className="min-w-0 flex-1 truncate text-[var(--text-secondary)]">{a.nome} <span className="text-[var(--text-muted)]">· saldo {brl(a.saldoBrl)}</span></span>{alocGeralModo === "MANUAL" ? <input inputMode="decimal" value={alocGeralManual[a.obrigacaoId] ?? ""} onChange={(e) => setAlocGeralManual((x) => ({ ...x, [a.obrigacaoId]: e.target.value }))} placeholder="0,00" className="w-28 rounded-[var(--radius-sm)] border border-[var(--border-default)] bg-[var(--surface-input)] px-2.5 py-1.5 text-right text-sm text-[var(--text-primary)] outline-none focus:border-[var(--info)]" /> : <span className="text-[var(--text-primary)]">{brl(a.valor)}</span>}</div>))}
                      <div className={`mt-1 flex items-center justify-between border-t border-[var(--border-default)] pt-1.5 text-xs ${Math.abs(somaGeral - recebido) < 0.02 ? "text-[var(--success)]" : "text-[var(--danger)]"}`}><span>Alocado</span><span>{brl(somaGeral)} / {brl(recebido)}</span></div>
                    </div>
                    <p className="flex items-center gap-1.5 text-[11px] text-[var(--text-muted)]"><InfoIcon className="h-3.5 w-3.5" /> Prévia: cada participante recebe a fração acima, aplicada na cobrança dele.</p>
                  </div>
                ) : (<>
                <div className="flex flex-wrap gap-2">
                  {([["NESTA", "Nesta cobrança"], ["PROXIMAS", "Próximas cobranças"], ["MAIS_ANTIGA", "Cobrança mais antiga"], ["AUTOMATICA", "Distribuição automática"], ["MANUAL", "Seleção manual"]] as const).map(([v, lb]) => (
                    <button key={v} onClick={() => setPolitica(v)} className={`rounded-[var(--radius-sm)] border px-3 py-1.5 text-xs font-medium ${politica === v ? "" : "border-[var(--border-default)] text-[var(--text-secondary)] hover:bg-[var(--surface-hover)]"}`} style={politica === v ? { borderColor: "color-mix(in srgb, var(--accent-primary) 60%, transparent)", background: "color-mix(in srgb, var(--accent-primary) 10%, transparent)", color: "var(--accent-hover)" } : undefined}>{lb}</button>
                  ))}
                </div>
                {politica === "MANUAL" ? (
                  <div className="mt-3 rounded-[var(--radius-sm)] bg-[var(--surface-secondary)] p-3">
                    <p className="mb-2 text-xs text-[var(--text-muted)]">Distribua o valor entre as parcelas pendentes (a soma deve ser igual ao total informado):</p>
                    <div className="space-y-1.5">{parcelasManuais.map((p: any) => (
                      <div key={p.id} className="flex items-center justify-between gap-2 text-sm">
                        <span className="min-w-0 flex-1 truncate text-[var(--text-secondary)]">Parcela {p.numero}/{p.totalParcelas} · vence {dataBR(p.vencimento)} · saldo {brl(p.saldoBrl ?? 0)}</span>
                        <input inputMode="decimal" value={alocManual[p.id] ?? ""} onChange={(e) => setAlocManual((a) => ({ ...a, [p.id]: e.target.value }))} placeholder="0,00" className="w-28 rounded-[var(--radius-sm)] border border-[var(--border-default)] bg-[var(--surface-input)] px-2.5 py-1.5 text-right text-sm text-[var(--text-primary)] outline-none focus:border-[var(--info)]" />
                      </div>
                    ))}{!parcelasManuais.length && <p className="text-xs text-[var(--text-muted)]">Sem parcelas em aberto — o valor será aplicado ao saldo.</p>}</div>
                    {parcelasManuais.length > 0 && <div className={`mt-2 flex items-center justify-between text-xs ${Math.abs(somaManual - recebido) < 0.01 ? "text-[var(--success)]" : "text-[var(--danger)]"}`}><span>Alocado</span><span>{brl(somaManual)} / {brl(recebido)}</span></div>}
                  </div>
                ) : (
                  <p className="mt-2 flex items-center gap-1.5 rounded-[var(--radius-sm)] bg-[var(--surface-secondary)] px-3 py-2 text-xs text-[var(--text-muted)]"><InfoIcon className="h-3.5 w-3.5" /> {politica === "NESTA" ? "O valor será aplicado prioritariamente nesta cobrança." : politica === "PROXIMAS" ? "O valor será direcionado às próximas cobranças." : politica === "MAIS_ANTIGA" ? "O valor quitará primeiro a cobrança mais antiga em aberto." : "O valor será distribuído proporcionalmente entre as parcelas em aberto."}</p>
                )}
                </>)}
              </section>
              ...
              <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
                <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                  <h2 className="text-sm font-semibold text-[var(--text-secondary)]">5. Documentos e Comprovantes</h2>
                  <p className="mb-3 text-xs text-[var(--text-muted)]">Anexe os comprovantes deste pagamento.</p>
                  <div onClick={() => fileRef.current?.click()} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); onFiles(e.dataTransfer.files) }} className="cursor-pointer rounded-[var(--radius-sm)] border border-dashed border-[var(--border-strong)] bg-[var(--surface-secondary)] px-4 py-6 text-center hover:border-[var(--border-strong)]">
                    {subindo ? <Loader2 className="mx-auto h-5 w-5 animate-spin text-[var(--text-muted)]" /> : <Upload className="mx-auto h-5 w-5 text-[var(--text-muted)]" />}
                    <p className="mt-2 text-xs text-[var(--text-muted)]">Arraste arquivos aqui ou <span className="text-[var(--info)]">selecionar arquivos</span></p>
                    <p className="text-[10px] text-[var(--text-muted)]">PDF, JPG, PNG ou DOCX até 20MB</p>
                  ...
                  {comprovantes.length > 0 && (
                    <div className="mt-3 space-y-1.5">
                      <p className="text-[11px] uppercase tracking-wide text-[var(--text-muted)]">Arquivos anexados ({comprovantes.length})</p>
                      {comprovantes.map((c, i) => (
                        <div key={i} className="flex items-center gap-2 rounded-[var(--radius-sm)] bg-[var(--surface-secondary)] px-3 py-2 text-sm">
                ...
                <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                  <h2 className="text-sm font-semibold text-[var(--text-secondary)]">6. Observações</h2>
                  <p className="mb-3 text-xs text-[var(--text-muted)]">Informações internas sobre este recebimento.</p>
                  ...
                  <p className="mt-1 text-right text-[10px] text-[var(--text-muted)]">{observacao.length}/1000</p>
                </section>
              </div>
            </div>
            ...
            <div className="space-y-4 lg:sticky lg:top-4 lg:self-start">
              <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                <h2 className="mb-3 text-sm font-semibold text-[var(--text-secondary)]">Resumo do Recebimento</h2>
                <dl className="space-y-2 text-sm">
                  ...
                  <Row k="Valor Líquido Recebido" cls="font-semibold text-[var(--info)]">{brl(recebido)}</Row>
                  <Row k="Saldo Restante" cls={!temValor ? "text-[var(--text-secondary)]" : saldoRestante > 0.005 ? "text-[var(--danger)]" : "text-[var(--success)]"}>{brl(saldoRestante)}</Row>
                  <div className="flex items-center justify-between pt-1"><dt className="text-[var(--text-muted)]">Situação Final</dt><dd><span className="rounded-[var(--radius-sm)] px-2 py-0.5 text-[11px] font-semibold" style={temValor ? statusStyle(situacao) : { background: "var(--surface-hover)", color: "var(--text-secondary)" }}>{situacao === "INICIAL" ? "Aguardando dados do recebimento" : situacao === "QUITADO" ? "Quitada" : situacao === "EXCEDENTE" ? "Excedente" : "Parcial"}</span></dd></div>
                </dl>
              ...
              <section className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-primary)] p-5">
                <h2 className="mb-3 text-sm font-semibold text-[var(--text-secondary)]">Impacto Financeiro</h2>
                {!temValor ? (
                  <p className="flex items-start gap-2 text-xs text-[var(--text-muted)]"><InfoIcon className="mt-0.5 h-3.5 w-3.5 shrink-0" /> Preencha a composição do pagamento para visualizar o impacto.</p>
                ) : (
              ...
              {erroSubmit && <div className="rounded-[var(--radius-sm)] border p-3 text-xs text-[var(--danger)]" style={{ borderColor: "color-mix(in srgb, var(--danger) 30%, transparent)", background: "color-mix(in srgb, var(--danger) 10%, transparent)" }}>{erroSubmit}</div>}
              {!valido && <div className="rounded-[var(--radius-sm)] border border-[var(--border-default)] bg-[var(--surface-secondary)] p-3 text-xs text-[var(--text-muted)]"><p className="mb-1 font-medium text-[var(--text-secondary)]">Pendências:</p><ul className="list-inside list-disc space-y-0.5">{pendencias.map((p, i) => <li key={i}>{p}</li>)}</ul></div>}
            </div>
          </div>
        )}
      </div>
...
function V({ children, cls = "text-[var(--text-primary)]" }: { children: React.ReactNode; cls?: string }) { return <div className={`text-sm font-semibold ${cls}`}>{children}</div> }
function Mini({ label, children, cls = "text-[var(--text-primary)]" }: { label: string; children: React.ReactNode; cls?: string }) { return <div><div className="text-[10px] uppercase tracking-wide text-[var(--text-muted)]">{label}</div><div className={`mt-0.5 text-sm font-semibold ${cls}`}>{children}</div></div> }
function Row({ k, children, cls = "text-[var(--text-primary)]" }: { k: string; children: React.ReactNode; cls?: string }) { return <div className="flex items-center justify-between"><dt className="text-[var(--text-muted)]">{k}</dt><dd className={cls}>{children}</dd></div> }
function Det({ icon: Ic, k, children }: { icon: any; k: string; children: React.ReactNode }) { return <div className="flex items-start gap-2"><Ic className="mt-0.5 h-4 w-4 shrink-0 text-[var(--text-muted)]" /><div className="min-w-0"><div className="text-[11px] text-[var(--text-muted)]">{k}</div><div className="truncate text-[var(--text-secondary)]">{children}</div></div></div> }
function Radio({ checked, onChange, children }: { checked: boolean; onChange: () => void; children: React.ReactNode }) { return <label className="inline-flex cursor-pointer items-center gap-1.5 text-sm text-[var(--text-secondary)]"><input type="radio" checked={checked} onChange={onChange} className="accent-[var(--info)]" /> {children}</label> }
