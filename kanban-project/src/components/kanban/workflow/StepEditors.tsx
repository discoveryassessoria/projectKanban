...
//   - EditorConferirEValidarCertidao (etapa 4/4 — conferência + validação, passo unificado)
//
// EditorConferirCertidao e EditorValidarCertidao (as etapas 5/6 de antes da
// unificação) foram removidos em 16/09/2026: zero definição e zero instância,
// em qualquer ciclo, em toda a produção — código morto confirmado por
// varredura no banco, não presunção.
...
function EditorShell({
  ...
  const content = (
    <>
      <div
        className="fixed inset-0 bg-[var(--overlay-modal)] z-[10004] transition-opacity"
        onClick={onClose}
      />
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      >
                        {ativo && (
                          <div className="w-2 h-2 rounded-full bg-[var(--surface-popover)]" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div
 ...
 * que cobre o passo inteiro. `FormValidarCertidao`/`EditorValidarCertidao`
 * (o caminho de um "Validar certidão" publicado como STEP PRÓPRIO, de versões
 * anteriores à unificação) foram removidos em 16/09/2026: confirmado no banco
 * que "validar_certidao" não tem nenhuma definição nem instância, em nenhum
 * ciclo, em toda a produção.
 *
