  ...
  if (!workflow) {
    return (
      <div className="p-5 space-y-4">
        <div className="bg-[#161b21] border border-white/10 rounded-xl p-6 text-center">
    ...
    <div className="p-5 space-y-4">
      ...
      <div className={`rounded-xl border p-4 ${
        isOverdue
          ...
          : "bg-[#161b21] border-white/10"
      }`}>
        <div className="text-[10px] font-bold uppercase tracking-wider text-white/45 mb-3">Resumo da operação</div>
          ...
          <Cell label="Responsável">
            {onAtribuir && usuarios && usuarios.length > 0 ? (
              <select
                ...
                className="w-full bg-[#161b21] border border-white/10 rounded px-1.5 py-1 text-[12.5px] text-white focus:outline-none focus:border-[#7dd3fc]/50"
        ...
        <button
          ...
          className="w-full inline-flex items-center justify-center gap-2 px-3 py-2 text-[12px] font-semibold text-white bg-[#2563eb] hover:bg-[#1d4ed8] rounded-md transition-colors"
        >
          Acessar painel completo da etapa
      ...
      {/* ============== BLOCO 2: IMPEDITIVOS ============== */}
      {blockers.length > 0 ? (
        <div className="bg-[#161b21] border border-white/10 rounded-xl p-4">
...
function ShortcutCard({
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`flex items-center justify-between gap-2 px-3 py-2.5 bg-[#161b21] border rounded-lg text-left transition-colors ${SEV_BORDER[sev]} ${
