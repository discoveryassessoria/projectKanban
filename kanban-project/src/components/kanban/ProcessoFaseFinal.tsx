...
export function ProcessoFaseFinal({ processoId, onConcluido }: Props) {
  ...
  if (!fase || !faseKey) {
    return <div className="rounded-xl border border-dashed border-[var(--border-default)] p-8 text-center text-sm text-[var(--text-secondary)]">Este processo não está numa fase final.</div>
  }
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)]">Objetivo: {cfg.obj}</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <div className="rounded-lg border border-[var(--border-default)] bg-[var(--surface-popover)] px-3 py-2 text-center">
            ...
            <div className="text-[11px] text-[var(--text-secondary)] whitespace-nowrap">Etapas</div>
          </div>
          <div className="rounded-lg border border-[var(--border-default)] bg-[var(--surface-popover)] px-3 py-2 text-center">
          ...
          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${
            concluida ? "bg-green-50 text-green-700" : "bg-sky-50 text-sky-700"}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${concluida ? "bg-green-50" : "bg-sky-400"}`} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr,300px] gap-4">
        <div className="space-y-4">
                  ...
                  <div key={s.id} className={`flex items-start ${i < fase.workflow.length - 1 ? "flex-1" : ""}`}>
                    <button type="button" disabled={!active} onClick={() => active && setModalStep(s.id)}
                      ...
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                        isDone ? "bg-green-50 text-white" : active ? "bg-[var(--action-primary)] text-white" : "bg-[var(--surface-tertiary)] text-[var(--text-secondary)]"}`}>
                        {isDone ? <Check className="w-4 h-4" /> : i + 1}
                      </div>
                      <div className="mt-1.5 text-[11px] font-medium text-white/80 leading-tight">{s.title}</div>
                      <div className={`text-[10px] ${isDone ? "text-green-700" : active ? "text-sky-700" : "text-[var(--text-muted)]"}`}>
                        {isDone ? "Concluído" : active ? "Em andamento" : "Pendente"}
                      </div>
                    </button>
                    {i < fase.workflow.length - 1 && <div className={`flex-1 h-0.5 mt-3.5 ${isDone ? "bg-green-50" : "bg-[var(--surface-tertiary)]"}`} />}
                  </div>
                )
              })}
            </div>
          ...
          <div className="rounded-xl border border-[var(--border-default)] bg-[var(--surface-secondary)] p-4 flex items-start gap-3">
            <div className="w-9 h-9 rounded-lg bg-[var(--surface-secondary)] flex items-center justify-center text-lg flex-shrink-0">{cfg.icon}</div>
          ...
          <div className="rounded-xl border border-[var(--border-default)] overflow-hidden">
            <div className="px-4 py-2.5 border-b border-[var(--border-default)] flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-[var(--text-secondary)] bg-[var(--surface-tertiary)] rounded-full px-2 py-0.5">{cfg.steps.length}</span>
            </div>
                ...
                return (
                  <div key={s.id} className="flex items-center gap-3 px-4 py-3">
                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold ${
                      isDone ? "bg-green-50 text-green-700" : active ? "bg-[var(--action-primary)] text-white" : "bg-[var(--surface-tertiary)] text-[var(--text-muted)]"}`}>{i + 1}</span>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-white/95">{s.title}</div>
                      <div className="text-[11px] text-[var(--text-secondary)]">{meta}</div>
                    </div>
                    {active && <button onClick={() => setModalStep(s.id)} className="text-xs font-semibold text-[var(--text-primary)] bg-[var(--surface-secondary)] hover:bg-[var(--surface-tertiary)] border border-[var(--border-default)] rounded-md px-2.5 py-1.5">Abrir etapa</button>}
                    {isDone && <span className="text-green-500"><Check className="w-4 h-4" /></span>}
                    {!isDone && !active && <span className="text-[var(--text-muted)]">🔒</span>}
                  </div>
                )
              })}
            </div>
        ...
        <aside className="space-y-4">
          <div className="rounded-xl border border-[var(--border-default)] bg-[var(--surface-popover)] p-4">
            ...
            <button onClick={() => activeStep ? setModalStep(activeStep.id) : setAviso("Todas as etapas concluídas.")}
              ...
              <ArrowRight className="w-4 h-4 text-[var(--text-muted)]" /> Abrir etapa atual
            </button>
          </div>
          <div className="rounded-xl border border-[var(--border-default)] bg-[var(--surface-popover)] p-4">
...
function Res({ k, v }: { k: string; v: string }) {
  return <div className="flex items-center justify-between"><span className="text-[var(--text-secondary)]">{k}</span><b className="text-white/95">{v}</b></div>
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-[var(--surface-popover)] rounded-xl shadow-xl max-h-[85vh] flex flex-col">
        <div className={`flex items-start justify-between px-5 py-4 border-b ${isLast ? "border-[var(--border-default)]" : "border-[var(--border-default)]"}`}>
          <div>
            <div className={`text-[11px] font-semibold uppercase tracking-wider ${isLast ? "text-red-700" : "text-sky-700"}`}>Etapa {n} de {cfg.steps.length} · {cfg.phaseName}</div>
            <h3 className="text-base font-bold text-white/95 mt-0.5">{sdef.title}</h3>
            <p className="text-xs text-[var(--text-secondary)] mt-0.5">{sdef.desc}</p>
          </div>
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          {stepId === "montar_dossie_final" && (
            <>
              <div className="rounded-lg border border-[var(--border-default)] bg-[var(--surface-secondary)] px-3 py-2.5 text-xs text-white/68 flex items-center gap-2">
                <FileText className="w-4 h-4 text-[var(--text-muted)]" /> Reúna todos os documentos apostilados e traduzidos do processo num dossiê único.
              </div>
          ...
          {stepId === "acompanhar_andamento" && (
            <>
              ...
              <div className="text-[11px] text-[var(--text-secondary)]">Conclua quando o acompanhamento estiver em dia e a decisão for aguardada.</div>
            </>
          )}
              ...
              <div className="space-y-2">
                {([["deferido", "✓ Deferido", "Reconhecimento concedido · segue para Finalizado"],
                  ...
                  <button key={v} type="button" onClick={() => setDecision(v)}
                    className={`w-full text-left border rounded-lg px-3 py-2.5 ${decision === v ? (v === "deferido" ? "border-green-200 bg-green-50" : "border-[var(--border-strong)] bg-[var(--surface-secondary)]") : "border-[var(--border-default)]"}`}>
          ...
          {stepId === "encerrar_processo" && (
            <>
              <div className="rounded-lg border border-[var(--border-default)] bg-[var(--surface-secondary)] px-3 py-2.5 text-sm text-white/80">📦 Ao encerrar, o processo é arquivado como <b>Finalizado</b> e sai do fluxo operacional.</div>
  ...
  return (
    <div>
      <label className="text-xs font-semibold text-white/80 flex items-center gap-1.5 mb-1">
        {label}{required && <span className="text-[10px] font-bold text-red-700 bg-red-50 rounded px-1.5 py-0.5">Obrigatório</span>}
      </label>
      {children}
    </div>
  )
...
function Chk({ on, onClick, children }: { on: boolean; onClick: () => void; children: ReactNode }) {
  return (
    <button type="button" onClick={onClick}
      className={`w-full flex items-center gap-2 border rounded-lg px-3 py-2 text-sm text-left ${on ? "border-green-200 bg-green-50 text-green-700" : "border-[var(--border-default)] text-white/80"}`}>
      <span className={`w-4 h-4 rounded flex items-center justify-center ${on ? "bg-green-50 text-white" : "border border-[var(--border-default)]"}`}>{on && <Check className="w-3 h-3" />}</span>
      {children}
    </button>
  )
}
function FakeUpload({ label }: { label: string }) {
  ...
  return (
    <button type="button" onClick={() => setOk(true)}
      className={`w-full inline-flex items-center gap-2 text-sm font-semibold rounded-md border px-3 py-2.5 ${ok ? "border-green-200 bg-green-50 text-green-700" : "border-[var(--border-default)] text-white/80"}`}>
