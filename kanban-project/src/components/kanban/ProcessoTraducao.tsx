...
// 6 etapas internas (motor) — a barra visual mostra 5: "Receber traduções"
// cobre tanto o "aguardar_retorno_tradutor" quanto o "receber_traducoes"
// internos (o mockup não tem uma etapa visual só de espera).
const VISUAL_STEPS = ["Montar pasta", "Enviar ao tradutor", "Receber traduções", "Conferir traduções", "Validar pasta"]
// internalStepId → índice visual (0-4)
const VISUAL_INDEX: Record<string, number> = {
  montar_pasta_traducao: 0,
  enviar_tradutor_juramentado: 1,
  aguardar_retorno_tradutor: 2,
  receber_traducoes: 2,
  conferir_traducoes: 3,
  validar_pasta_traduzida: 4,
}
...
export function ProcessoTraducao({ processoId, onConcluido }: Props) {
  ...
  const { dados, carregando: loading, recarregar: carregar } =
    useApi<RespostaGet>(`/api/processos/${processoId}/traducao`)
  ...
  const mutarPasta = async (acao: "adicionar" | "remover", documentoIds: number[]) => {
    setErroAcao(null)
    try {
      const res = await fetch(`/api/processos/${processoId}/traducao/pasta/documentos`, {
  ...
  const podeEditarPasta = pasta.currentStep === "montar_pasta_traducao"
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-[var(--border-default)] bg-[var(--surface-popover)] p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white/95">Tradução Juramentada</h2>
        ...
        <div className="mt-4 flex flex-wrap gap-6">
          ...
          <CampoContexto label="Tradutor">
            {pasta.translatorName
              ? <span className="text-sm font-semibold text-white/95">{pasta.translatorName}</span>
              : <button onClick={() => setModalStep("enviar_tradutor_juramentado")} className="text-sm font-semibold text-[var(--accent-text)] hover:underline">Selecionar</button>}
          </CampoContexto>
          <CampoContexto label="Origem">
            <span className="text-sm font-semibold text-white/95">{pasta.sourceLanguage}</span>
          </CampoContexto>
          <CampoContexto label="Destino">
            <span className="text-sm font-semibold text-white/95">{paisDestino?.flag ? `${paisDestino.flag} ` : ""}{pasta.targetLanguage}</span>
          </CampoContexto>
          <CampoContexto label="Prazo estimado">
            <span className="text-sm font-semibold text-white/95">{fmtDate(pasta.expectedDate)}</span>
            {podeEditarPasta && <button onClick={() => setModalStep("enviar_tradutor_juramentado")} className="ml-1.5 text-xs font-semibold text-[var(--accent-text)] hover:underline">Editar</button>}
          </CampoContexto>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        ...
        <Kpi icon="📤" value={totais?.enviados ?? 0} label="Enviados ao tradutor" />
        <Kpi icon="📥" value={totais?.recebidos ?? 0} label="Traduções recebidas" />
        ...
        {podeEditarPasta && (
          <div className="flex items-center gap-3 flex-shrink-0">
            <div className="rounded-lg border border-[var(--border-default)] bg-[var(--surface-popover)] px-3 py-2">
              ...
              <div className="text-[11px] text-[var(--text-secondary)]">{(totais?.naPasta ?? 0) > 0 ? "Pronta para envio ao tradutor" : "Adicione documentos aptos"}</div>
            </div>
            <button
              onClick={() => setModalStep("enviar_tradutor_juramentado")}
              ...
              <Send className="w-4 h-4" /> Enviar pasta ao tradutor
            </button>
          </div>
        )}
      </div>
      ...
      <div className="rounded-xl border border-[var(--border-default)] bg-[var(--surface-popover)] overflow-hidden">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-[10px] uppercase tracking-wider text-[var(--text-secondary)] bg-[var(--surface-secondary)]">
                    ...
                    <th className="text-left font-semibold px-3 py-2 whitespace-nowrap">Apto para tradução</th>
                    <th className="text-left font-semibold px-3 py-2 whitespace-nowrap">Na pasta</th>
                    <th className="text-left font-semibold px-3 py-2 whitespace-nowrap">Status da tradução</th>
                    ...
                    return (
                      <Fragment key={p.pessoaId}>
                        ...
                        {!colapsada && p.documentos.map((d) => (
                          <LinhaDocumento key={d.documentoId} d={d} podeEditarPasta={podeEditarPasta} selecionado={selecionados.has(d.documentoId)} onToggle={() => toggleSelecionado(d.documentoId)} labelStatus={TR_DOC_LABEL} />
                        ))}
                      </Fragment>
                    )
                  }) : (
                    todosDocs.filter(passaFiltro).map((d) => (
                      <LinhaDocumento key={d.documentoId} d={d} podeEditarPasta={podeEditarPasta} selecionado={selecionados.has(d.documentoId)} onToggle={() => toggleSelecionado(d.documentoId)} labelStatus={TR_DOC_LABEL} mostrarPessoa />
                    ))
                  )}
        ...
        {aba === "resumo" && <ResumoDaPasta docs={pasta.documentos} labelStatus={TR_DOC_LABEL} />}
        {aba === "historico" && <HistoricoInline processoId={processoId} />}
      </div>

      {erroAcao === null && aviso && <div className="bg-[var(--surface-secondary)] border border-[var(--border-default)] rounded-lg px-4 py-3 text-sm text-[var(--text-secondary)]">{aviso}</div>}
...
function ResumoDaPasta({ docs, labelStatus }: { docs: TrDoc[]; labelStatus: Record<string, string> }) {
        ...
        <tbody className="divide-y divide-white/10">
          {docs.map((d) => (
            <tr key={d.id} className="hover:bg-[var(--surface-secondary)]">
              <td className="px-3 py-2.5 font-semibold text-white/95">{d.pessoaNome}</td>
              <td className="px-3 py-2.5">{d.documentoTitulo}</td>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-[var(--surface-popover)] rounded-xl shadow-[var(--elev-3)] max-h-[85vh] flex flex-col">
        <div className="flex items-start justify-between px-5 py-4 border-b border-[var(--border-default)]">
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-wider text-[var(--text-secondary)]">Etapa {num} de 5 · Workflow da Tradução</div>
                ...
                <div className="flex flex-wrap gap-2">
                  {ENVIO_METODOS.map(([v, l]) => (
                    <button key={v} type="button" onClick={() => setMetodo(v)}
                      className={`px-3 py-1.5 text-xs font-semibold rounded-md border ${metodo === v ? "border-[var(--border-strong)] bg-[var(--surface-secondary)] text-white" : "border-[var(--border-default)] text-white/80"}`}>
                      {l}
                    </button>
                  ))}
                </div>
              ...
              {docs.map((d) => (
                <div key={d.id} className="border border-[var(--border-default)] rounded-lg p-3">
                  <div className="flex items-baseline gap-2 mb-2">
                    ...
                    <small className="text-[11px] text-[var(--text-secondary)]">{d.pessoaNome}</small>
                  </div>
