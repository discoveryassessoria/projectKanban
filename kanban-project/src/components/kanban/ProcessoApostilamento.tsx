...
export function ProcessoApostilamento({ processoId, onConcluido }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/processos/${processoId}/apostilamento`, { headers: authHeaders(), signal: ac.signal })
      .then((res) => res.json())
      .then((data) => {
      ...
      .catch(() => { if (!ac.signal.aborted) setErro("Erro ao carregar a pasta de apostilamento.") })
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr,300px] gap-4">
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-white/10 overflow-hidden">
            ...
            {docs.length === 0 ? (
              <div className="p-8 text-center text-sm text-white/55">Nenhum documento final para apostilamento.</div>
            ) : (
                    ...
                    {docs.map((it) => (
                      <tr key={it.id} className="hover:bg-[#20262e] align-top">
                        ...
                        <td className="px-3 py-2.5">
                          ...
                          {it.apostilleNumber && <div className="text-[11px] text-white/55">Apostila {it.apostilleNumber}</div>}
                        </td>
              ...
              <div className="space-y-2">
                {docs.map((d) => (
                  <div key={d.id} className="flex items-center gap-3 border border-white/10 rounded-lg px-3 py-2">
                    ...
                    <span className="text-[11px] text-white/55">{AP_DOC_LABEL[d.status] || d.status}</span>
                  </div>
                ))}
