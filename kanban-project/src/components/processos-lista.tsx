...
import type { AtividadeWithStatus, Status, Contratante } from "@/src/types/kanban"
...
export function ProcessosLista({ 
  ...
  const paginatedAtividades = filteredAtividades.slice(startIndex, startIndex + itemsPerPage)
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="text-sm text-white/60">
          {filteredAtividades.length} processo(s) encontrado(s)
        </div>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/10">
              ...
              <th className="text-left py-3 px-4 text-white/60 font-medium text-sm">Responsável</th>
            ...
            {paginatedAtividades.map((atividade) => (
              ...
              >
                <td className="py-3 px-4">
                  <div>
                    <span className="text-white font-medium">{atividade.nome}</span>
                    {atividade.descricao && (
                ...
                <td className="py-3 px-4">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <div className={`h-2 w-2 rounded-full ${getStatusColor(atividade.statusId)}`} />
                      <span className="text-white/80 text-sm">{getStatusNome(atividade.statusId)}</span>
                    </div>
                    {/* Barra de progresso */}
                    <div className="w-32 h-1.5 bg-white/10 rounded-full overflow-hidden">
                      <div 
                        className={`h-full ${getStatusColor(atividade.statusId)} transition-all duration-300`}
                        style={{ width: `${getProgress(atividade.statusId)}%` }}
                      />
                    </div>
                  </div>
                </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between pt-4 border-t border-white/10">
          <span className="text-sm text-white/60">
            Mostrando {startIndex + 1} a {Math.min(startIndex + itemsPerPage, filteredAtividades.length)} de {filteredAtividades.length}
          </span>
          <div className="flex items-center gap-2">
            <Button
              size="sm"
